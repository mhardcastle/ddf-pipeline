...
option_list = (
    ( 'machine',    'NCPU_DDF',    int, getcpus(),'Number of CPUS to use for DDF'),
    ( 'machine',    'NCPU_killms', int, getcpus(),'Number of CPUS to use for KillMS' ),
    
    ( 'data',       'mslist',      str, None, 'Initial measurement set list to use -- must be specified' ),
    ( 'data',       'full_mslist', str, None, 'Full-bandwidth measurement set to use for final step, if any' ),
    ...
    ( 'solutions',  'cluster_cut_deg',  float, 0.0,   'Maximum distance (deg) of directions from each MS phase centre when solving per-MS (0 disables pruning)' ),
    ...
    ( 'image',      'wscms_neg_model_mode',   list,  [1, 1, 1, 1], "WSCMS2 negative-component policy per step: 0=positive-only, 1=safe (model flux + scale cap), 2=free (default: safe)"),
    ...
    ( 'masking',    'wscms_compact_margin',      float, None, 'WSCMS2 history margin: min ratio of biased scale-0 to scale-s response for compact-test (default: 1.0 if unset)'),
    ( 'masking',    'wscms_rescue_rms_factor',   float, None, 'WSCMS2 rescue threshold at the automask freeze (default: same as automask_rms_factor)'),
    ...
    ( 'masking',    'extended_size',             int,   None,  'If generating a mask from the bootstrap low-res images, use islands larger than this size in pixels' ),
    ( 'masking',    'extended_rms',              float, 3.0,   'Threshold value defining an island in the extended mask'),
