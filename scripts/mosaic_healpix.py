...
def make_header(healpix,cellsize,resolution,history=None):
    # construct template FITS header
    centre=hp.healpix_to_skycoord(healpix)
    corners=hp.boundaries_skycoord(healpix, step=1)
    dra,ddec=centre.spherical_offsets_to(corners)
    dra=dra.value
    ddec=ddec.value
    #ddec=corners.dec.value-centre.dec.value
    #dra=(corners.ra.value-centre.ra.value)*np.cos(np.radians(centre.dec.value))
    ...
    header['OBJECT']='HP-'+str(healpix)
    if history is not None:
        for h in history:
            header.add_history(h)
    return header,himsize

def blank_mosaic(healpix,imname):
    hdu=fits.open(imname)
    himsize,_=hdu[0].data.shape
    wcs=WCS(hdu[0].header)
    x=np.array(list(range(0,himsize)))
    xv, yv = np.meshgrid(x, x)
    x=xv.flatten()
    y=yv.flatten()
    result=wcs.pixel_to_world(x,y)
    pixels=hp.lonlat_to_healpix(result.ra,result.dec).reshape((himsize,himsize))
    hdu[0].data[pixels!=healpix]=np.nan
...
if __name__=='__main__':
    ...
    parser.add_argument('--healpixmap', type=str, help='Healpix lookup map JSON')
    parser.add_argument('healpixnumber', type=int, help='Healpix number')
    ...
    parser.add_argument('--skip-done',dest='skip_done',action='store_true',help='Skip recreating the mosaic(s) if they exist')

    
    args = parser.parse_args()
    healpix = args.healpixnumber
    ...
    with open(args.healpixmap,'r') as infile:
        d=json.load(infile)
        
    mosaicpointings=d[str(healpix)]
    print(mosaicpointings)
    ...
    if not(args.skip_done) or not(os.path.isfile('mosaic.fits')):
        if check_convolve:
        ...
        if not(args.no_highres):
            ...
            header,himsize=make_header(healpix,1.5,high_resolution,history=checksums)
            ...
            blank_mosaic(healpix,mosname)

        if args.do_lowres:
            print('Making the low-resolution mosaic...')
            header,himsize=make_header(healpix,4.5,20.0,history=checksums)
            mos_args.header=header
            ...
            blank_mosaic(healpix,'low-mosaic.fits')
        ...
        if args.do_wsclean:
            ...
            blank_mosaic(healpix,'vlow-mosaic.fits')
        ...
        if args.do_stokesV:
            ...
            blank_mosaic(healpix,'stokesV-mosaic.fits')
