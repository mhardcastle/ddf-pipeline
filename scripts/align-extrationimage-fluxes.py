...
def plot_best_fit(X, y, xaxis, model):
    ...
    print(
        ...
        "intercept",
        intercept,
        "Mean MAE: %.3f (%.3f)" % (mean(results), std(results)),
    )
...
parser.add_argument(
    "catalogue",
    type=str,
    help="The LoTSS-DR2 catalogue or catalogue from another field",
)
...
parser.add_argument(
    ...
    type=str,
    default="LoTSS-DR2",
    help="field name if not LoTSS-DR2 but using surveys database",
)
parser.add_argument(
    "--nodatabase", help="Do not use LOFAR surveys fields database", action="store_true"
)
parser.add_argument(
    "--fieldfactor",
    help="Scaling factor of the field if not using surveys database",
    type=float,
)
...
if not os.path.exists(infile.replace(".fits", "cat.srl.fits")):
    ...
    img.write_catalog(
        outfile=infile.replace(".fits", "cat.srl.fits"),
        catalog_type="srl",
        format="fits",
        correct_proj="True",
    )
    img.write_catalog(
        outfile=infile.replace(".fits", "cat.srl.reg"),
        catalog_type="srl",
        format="ds9",
        correct_proj="True",
    )
...
if fieldname == "LoTSS-DR2":
    ...
    if nodatabase:
        ...
        lotssdr2["Peak_flux"] = (
            lotssdr2["Peak_flux"] * fieldfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat
        lotssdr2["Total_flux"] = (
            lotssdr2["Total_flux"] * fieldfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat
        lotssdr2["E_Peak_flux"] = (
            lotssdr2["E_Peak_flux"] * fieldfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat
        lotssdr2["E_Total_flux"] = (
            lotssdr2["E_Total_flux"] * fieldfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat
    else:
        ...
        lotssdr2["Peak_flux"] = (
            lotssdr2["Peak_flux"] * nvssfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat
        lotssdr2["Total_flux"] = (
            lotssdr2["Total_flux"] * nvssfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat
        lotssdr2["E_Peak_flux"] = (
            lotssdr2["E_Peak_flux"] * nvssfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat
        lotssdr2["E_Total_flux"] = (
            lotssdr2["E_Total_flux"] * nvssfactor * 1000.0
        )  # 1000 scaling is to match mJy which is the units of LoTSS-DR2 cat


# lotssdr2=select_isolated_sources(lotssdr2,45)
print("isolated sources", len(lotssdr2))
lotsssnr = lotssdr2["Peak_flux"] / (2.0 * lotssdr2["E_Peak_flux"]) + lotssdr2[
    "Total_flux"
] / (2.0 * lotssdr2["E_Total_flux"])
...
cutoutsnr = cutout["Peak_flux"] / (2.0 * cutout["E_Peak_flux"]) + cutout[
    "Total_flux"
] / (2.0 * cutout["E_Total_flux"])
...
for model in get_models():
    ...
    if mae < bestmae:
        bestmodel, bestgradient, bestintercept, bestmae, bestmaestd = (
            modelname,
            gradient,
            intercept,
            mae,
