...
def losotolofarbeam(
    ...
    numants = pt.taql("select gcount(*) as numants from " + ms + "::ANTENNA").getcol(
        "numants"
    )[0]
    times = soltab.getAxisValues("time")

    for vals, _, selection in soltab.getValuesIter(
...
def flagms_startend(ms, tecsolsfile, tecsolint):
    ...
    cmd = (
        ...
        + " ' select from "
        + ms
        + " where TIME in (select distinct TIME from "
        + ms
    )
...
def plotimage(fitsimagename, outplotname, mask=None, rmsnoiseimage=None):
    ...
    f.set_title(
        fitsimagename + " (noise = {} mJy/beam)".format(round(imagenoiseinfo * 1e3, 3))
    )
    f.add_beam()
...
def reweight(
    mslist, pixsize, imsize, channelsout, niter, robust, multiscale=False, fitsmask=None
):
    """
    ...
    for ms in mslist:
        ...
        makeimage(
            ...
            imsize,
            channelsout,
            np.int(old_div(niter, (len(mslist) ** (1.0 / 3.0)))),
            robust,
...
def determinesolints(
    ...
    """
    if (
        os.path.isfile("nchan_phase.p")
        and os.path.isfile("solint_phase.p")
        and os.path.isfile("solint_ap.p")
        and os.path.isfile("nchan_ap.p")
    ):
        ...
        logging.info(
            "Noise inceases by a factor of: "
            + str(declf)
            + " for this observations at delclination [deg] "
            + str(decl)
        )
        ...
        for ms in mslist:
            ...
            nchan_phase, solint_phase, solint_ap = calculate_solintnchan(
                old_div(csf, declf)
            )
            solint_phase = 2 * solint_phase
            nchan_ap = nchan_phase  # keep the same
            logging.info(
                "MS, NCHAN, SOLINT_PHASE, SOLINT_AP: "
                + str(ms)
                + " "
                + str(nchan_phase)
                + " "
...
def findrms(mIn, maskSup=1e-7):
    ...
    diff = 1e-1
    cut = 3.0
    bins = np.arange(np.min(m), np.max(m), (np.max(m) - np.min(m)) / 30.0)
    med = np.median(m)
    for i in range(10):
        ind = np.where(np.abs(m - med) < rmsold * cut)[0]
        rms = np.std(m[ind])
        if np.abs(old_div((rms - rmsold), rmsold)) < diff:
            break
        rmsold = rms
    return rms
...
def getimsize(boxfile, cellsize=1.5):
    ...
    xs = np.ceil(old_div((r[0].coord_list[2]) * 1.6 * 3600.0, cellsize))
    ...
    if (soltype == "complexgain") and (preapplyphase == True):
        ...
        if TEC == True:
            ...
            cmd += (
                "ac0.parmdb=phaseonly"
                + parmdb
                + " ac0.type=applycal ac0.correction=phase000 "
            )
            cmd += (
                "ac1.parmdb=phaseonly"
                + parmdb
                + " ac1.type=applycal ac1.correction=tec000 "
            )
            cmd += (
                "ac2.parmdb=" + parmdb + " ac2.type=applycal ac2.correction=phase000 "
            )
            cmd += (
                "ac3.parmdb="
                + parmdb
                + " ac3.type=applycal ac3.correction=amplitude000 "
            )
...
def change_refant(parmdb, soltab):
    ...
    refant = " "
    if (
        (old_div(np.float(len(idx0)), np.float(np.size(weights[:, :, 0, :, :])))) > 0.5
    ) or (
        (old_div(np.float(len(idxnan)), np.float(np.size(weights[:, :, 0, :, :]))))
        > 0.5
    ):
            ...
            print(
                ...
                (
                    (
                        old_div(
                            np.float(len(idx0)),
                            np.float(np.size(weights[:, :, antennaid + 1, :, :])),
                        )
                    )
                ),
            )
...
def calculate_solintnchan(compactflux):
    ...
    solint_ap = 120.0 / (compactflux ** (1.0 / 3.0))  # do third power-scaling
    # print solint_ap
    if solint_ap < 60.0:
        solint_ap = 60.0  # shortest solint_ap allowed
    if solint_ap > 180.0:
        solint_ap = 180.0  # longest solint_ap allowed
...
def determine_compactsource_flux(fitsimage):
    ...
    avgbeam = 3600.0 * 0.5 * (bmaj + bmin)
    pixsize = 3600.0 * (hdul[0].header["CDELT2"])
    rmsbox1 = np.int(7.0 * avgbeam / pixsize)
    rmsbox2 = np.int((rmsbox1 / 10.0) + 1.0)

    img = bdsf.process_image(
        fitsimage, mean_map="zero", rms_map=True, rms_box=(rmsbox1, rmsbox2)
    )
    hdul.close()
...
def getdeclinationms(ms):
    ...
    t.close()
    return 360.0 * direction[1] / (2.0 * np.pi)
...
def declination_sensivity_factor(declination):
    ...
    """
    factor = 1.0 / (np.cos(2.0 * np.pi * (declination - 52.9) / 360.0) ** 2)
...
def flaghighgamps(parmdb, highampval=10.0):
    """
...
def normamps(parmdb):
    ...
    if len(parmdb) == 1:
        ...
        amps = H5.getSolset("sol000").getSoltab("amplitude000").getValues()[0]
        weights = (
            H5.getSolset("sol000").getSoltab("amplitude000").getValues(weight=True)[0]
        )
        idx = np.where(weights != 0.0)

        amps = np.log10(amps)
        logging.info(
            "Mean amplitudes before normalization: "
            + str(10 ** (np.nanmean(amps[idx])))
        )
        amps = amps - (np.nanmean(amps[idx]))
        logging.info(
            "Mean amplitudes after normalization: " + str(10 ** (np.nanmean(amps[idx])))
        )
        amps = 10 ** (amps)
        ...
        for i, parmdbi in enumerate(parmdb):
            ...
            ampsi = np.copy(
                H5.getSolset("sol000").getSoltab("amplitude000").getValues()[0]
            )
            weights = (
                H5.getSolset("sol000")
                .getSoltab("amplitude000")
                .getValues(weight=True)[0]
            )
            idx = np.where(weights != 0.0)
            logging.info(
                parmdbi
                + "  Normfactor: "
                + str(10 ** (np.nanmean(np.log10(ampsi[idx]))))
            )
            if i == 0:
...
def makeimage(
    ...
    baselineav = str(
        old_div(
            1.87e3 * 60000.0 * 2.0 * np.pi * np.float(1.5),
            (24.0 * 60.0 * 60 * np.float(imsize)),
        )
    )
    ...
    cmd += (
        "-mgain 0.8 -fit-beam -data-column " + imcol + " -join-channels -channels-out "
    )
    ...
    if deepmultiscale:
        ...
        if idg:
            cmdp += (
                "-use-idg -grid-with-beam -use-differential-lofar-beam -idg-mode cpu "
            )
    ...
    if idg:
        removenegativefrommodel(
            sorted(glob.glob(imageout + "-????-I-model*.fits"))
        )  # only Stokes I
    else:
...
def runDP3(
    ...
    if preapplyphase:  # APPLY FIRST
        ...
        cmd += "steps=[ddecal] " + "msout.storagemanager=dysco ddecal.type=ddecal "
        cmd += (
            "ddecal.maxiter=100 ddecal.tolerance=1.e-5 ddecal.propagatesolutions=True "
        )
        cmd += "ddecal.usemodelcolumn=True "
        cmd += "ddecal.nchan=" + str(nchan_ap) + " "
        cmd += (
            "ddecal.mode=" + soltype + " "
        )  # scalarphase, phaseonly, complexgain, tec, tecandphase
        cmd += "ddecal.h5parm=" + parmdb + " "
        cmd += "ddecal.solint=" + str(solint_ap) + " "
        if FOVedge:
            cmd += "ddecal.smoothnessconstraint=8e6 "
    ...
    if (soltype == "complexgain") and (preapplyphase == True):
        ...
        if FOVedge == True:
            ...
            flaghighgamps(parmdb, highampval=10.0)
        else:
            flaglowamps(parmdb, lowampval=0.1)
            flaghighgamps(parmdb, highampval=10.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fitsmask",
        help="fitsmask for deconvolution, if not provided use automasking",
        type=str,
    )
    parser.add_argument(
        "--H5sols",
        help="prefix name for H5 solution file, default=solsgrid",
        default="solsgrid",
        type=str,
    )
    parser.add_argument(
        "--imsize", help="image size, required if boxfile is not used", type=int
    )
    parser.add_argument(
        "-n", "--niter", help="niter, default=15000", default=15000, type=int
    )
    parser.add_argument(
        "--robust",
        help="Briggs robust paramter, default=-0.5",
        default=-0.5,
        type=float,
    )
    parser.add_argument(
        "--channelsout", help="channelsout, default=6", default=6, type=int
    )
    parser.add_argument(
        "-u",
        "--uvmin",
        help="inner uv-cut for calibration in lambda, default=350",
        default=350.0,
        type=float,
    )
    ...
    parser.add_argument(
        "--multiscale",
        help="use multiscale deconvolution, not recommended",
        action="store_true",
    )
    parser.add_argument(
        "--pixelscale",
        help="pixels size in arcsec, deafult=1.5",
        default=1.5,
        type=float,
    )
    parser.add_argument(
        "--idg", help="use the Image Domain gridder", action="store_true"
    )
    parser.add_argument(
        "--no-beamcor",
        help="do not correct the visilbities for the array factor",
        action="store_false",
    )
    parser.add_argument(
        "-i", "--imagename", help="imagename, default=image", required=True, type=str
    )
    parser.add_argument(
        "--start",
        help="start selfcal cycle at this iteration, default=0",
        default=0,
        type=int,
    )
    parser.add_argument(
        "--stop",
        help="stop selfcal cycle at this iteration, default=10",
        default=10,
        type=int,
    )
    parser.add_argument(
        "--no-smoothcal", help="median smooth amplitudes", action="store_false"
    )
    parser.add_argument(
        "--maskthreshold",
        help="threshold for MakeMask.py, default=5",
        default=5,
        type=int,
    )
    if getcpuworks:
    ...
    nchan_phase, solint_phase, solint_ap, nchan_ap = determinesolints(
        mslist, pixsize, imsize, channelsout, np.int(niter), robust, TEC
    )
    ...
    for i in range(args["start"], args["stop"]):
        ...
        if (i > 0) and (args["fitsmask"] == None):
            if idg:
                if os.path.isfile(
                    imageout + str(i - 1) + "-MFS-I-image.fits.mask.fits"
                ):
        ...
        if idg:
            plotimage(
                imageout + str(i) + "-MFS-I-image.fits",
                imageout + str(i) + ".png",
                mask=fitsmask,
                rmsnoiseimage=imageout + str(0) + "-MFS-I-image.fits",
            )
        else:
            plotimage(
                imageout + str(i) + "-MFS-image.fits",
                imageout + str(i) + ".png",
                mask=fitsmask,
                rmsnoiseimage=imageout + str(0) + "-MFS-image.fits",
            )
        ...
        for msnumber, ms in enumerate(mslist):
            if i < switchtogaincycle:
                runDP3(
                    ms,
                    np.int(solint_ap[msnumber]),
                    np.int(solint_phase[msnumber]),
                    np.int(nchan_phase[msnumber]),
                    np.int(nchan_ap[msnumber]),
                    ms + parmdb + str(i) + ".h5",
                    "scalarphase",
                    False,
                    uvmin=uvmin,
        ...
        if (
            (i == 0)
            or (i == switchtogaincycle)
            or (i == switchtogaincycle + 1)
            or (i == switchtogaincycle + 2)
            or (i == switchtogaincycle + 3)
            or (i == switchtogaincycle + 4)
        ):
            for msnumber, ms in enumerate(mslist):
                flagms_startend(
                    ms,
                    "phaseonly" + ms + parmdb + str(i) + ".h5",
                    np.int(solint_phase[msnumber]),
                )
