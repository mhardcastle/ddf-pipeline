...
def getimsize(image):
    ...
    if imsizeddf is None:
        print(
            "Could not determine the image size, should have been 20000(?) or 6000(?)"
        )
        sys.exit(1)
    if cellddf is None:
...
def mask_region_cube(infilename, ds9region, outfilename):
    ...
    manualmask = r.get_mask(
        hdu=hdu[0], shape=(hdu[0].header["NAXIS1"], hdu[0].header["NAXIS2"])
    )
...
def getregionboxcenter(regionfile, standardbox=True):
    ...
    if standardbox:
        if boxsizex != boxsizey:
            print(
                "Only a square box region supported, you have these sizes:",
                boxsizex,
                boxsizey,
            )
            sys.exit()
    ...
    regioncenter = (
        "{:12.8f}".format(ra) + "deg," + "{:12.8f}".format(dec) + "deg"
    ).replace(" ", "")
    return regioncenter
...
def SummaryToVersion(summaryFile):
    ...
    DFields = {
        "StrDate": "ddf-pipeline completed at ",
    ...
    for npField in DFields.keys():
        ...
        while iLine < len(L):
            ...
            if l.startswith(field):
                ...
                DOut[npField] = d
                if npField == "StrDate":
                    DOut["time"] = time.mktime(
                        datetime.datetime.strptime(d, "%Y-%m-%d %H:%M:%S").timetuple()
                    )
                break
            iLine += 1
...
parser.add_argument(
    "-b", "--boxfile", help="boxfile, required argument", required=True, type=str
)
parser.add_argument(
    "-m",
    "--mslist",
    help="DR2 mslist file, default=big-mslist.txt",
    default="big-mslist.txt",
    type=str,
)
parser.add_argument(
    "-c",
...
parser.add_argument(
    "-f", "--freqavg", help="channel averaging, default=4", default=4, type=int
)
parser.add_argument(
    "-t", "--timeavg", help="timesample averaging, default=2", default=2, type=int
)
...
if getcpuworks:
    ...
    parser.add_argument(
        "-n",
        "--ncpu",
        help="number of cpu to use, default=%i" % os.cpu_count(),
...
parser.add_argument(
    "--nodysco", help="Do not dysco compress output", action="store_false"
)
parser.add_argument(
    "--split", help="Do not concat but keep 10 SB blocks", action="store_true"
)
parser.add_argument(
    "--aoflaggerbefore",
...
parser.add_argument(
    "--aoflaggerafter",
    help="Do an extra round of AOflagger on averaged output data",
    action="store_true",
)
parser.add_argument(
    "--maxamplitude",
    help="flag amplitudes above this number, default=1e6",
    default=1.0e6,
    type=float,
)
...
parser.add_argument(
    "--noconcat", help="Stop after making the DATA_SUB column", action="store_true"
)
...
parser.add_argument(
    "--AlsoMakeResidualImage",
    help="Do residual image together with predict column (the .dirty.fits), for debugging",
    action="store_true",
)
parser.add_argument(
    "--keeplongbaselines",
...
parser.add_argument(
    "--h5solstring",
    help="HDF5 solution string, default=sol000/phase000+amplitude000",
    default="sol000/phase000+amplitude000",
    type=str,
)
parser.add_argument(
    "--DDESolutions-DDSols", help="optional harcoded DDF solution string", type=str
)
parser.add_argument(
    "--clustercat",
    help="Cluster/nodes npy file, default=image_dirin_SSD_m.npy.ClusterCat.npy",
    default="image_dirin_SSD_m.npy.ClusterCat.npy",
    type=str,
)
parser.add_argument(
    "--chunkhours",
    help="Data-ChunkHours for DDF.py, default=9.0",
    default=9.0,
    type=float,
)
parser.add_argument(
    "--dicomask",
    help="Mask for filtering the Dico model, default=None (automatically determined)",
    type=str,
)
parser.add_argument(
    "--indico",
    help="Input Dico model, default=None (automatically determined)",
    type=str,
)
parser.add_argument(
    "--nofixsym",
    help="Do not attempt to fix killms symlinks -- assume ok",
    action="store_true",
)
parser.add_argument(
    "--nopredict",
...
parser.add_argument(
    ...
    help="Only use WEIGHT_SPECTRUM, ignore IMAGING_WEIGHT",
    action="store_true",
)
parser.add_argument(
    "--HMPmodelfits", help="if provided, use this HMP model fits  for predict"
)
parser.add_argument(
    "--nosubtract",
...
parser.add_argument(
    "--overwriteoutput", help="Overwrite concat ms if it exists", action="store_true"
)
...
if boxfile != "fullfield":
    ...
    if len(r[:]) > 1:
        ...
        if args["adjustboxrotation"]:
            ...
            boxfile = adjustboxrotationlocalnorth(
                boxfile, fullmask
            )  # use adjustedbox.reg
...
keywords = parse_parset(
    [os.environ["DDF_DIR"] + "/DDFacet/DDFacet/Parset/DefaultParset.cfg"],
    use_headings=True,
)
...
print(
    "[date = %s] DDF version = %s , using %s"
    % (DOut["StrDate"], DOut["v_DDF"], StrBeamAt)
)
...
if dopredict:
    ...
    if holesfixed:
        ...
        if args["h5sols"] != None:
            if not args["useHMP"]:
                run(
                    "DDF.py --Output-Name=image_dd_SUB --Data-ChunkHours="
                    + str(args["chunkhours"])
                    + " --Data-MS="
                    + args["mslist"]
                    ...
                    + " --Facets-CatNodes="
                    + clustercat
                    + " --Beam-CenterNorm=1 --Deconv-Mode SSD --Beam-Model=LOFAR "
                    + beamoption
                    + " --Weight-Robust "
                    + str(robust)
                    + " --Image-NPix="
                    + str(imagenpix)
                    + " --CF-wmax 50000 --CF-Nw 100 --Output-Also onNeds --Image-Cell "
                    + str(imagecell)
                    + " --Facets-NFacets=11 --SSDClean-NEnlargeData 0 --Freq-NDegridBand 1 --Beam-NBand 1 --Facets-DiamMax 1.5 --Facets-DiamMin 0.1 --Deconv-RMSFactor=3.000000 --SSDClean-ConvFFTSwitch 10000 --Data-Sort 1 --Cache-Dir=. --Log-Memory 1 --Cache-Weight=reset --Output-Mode="
                    + OutputMode
                    + " --Output-RestoringBeam 6.000000 --Freq-NBand=2 --RIME-DecorrMode=FT --SSDClean-SSDSolvePars [S,Alpha] --SSDClean-BICFactor 0 --Mask-Auto=1 --Mask-SigTh=5.00 --Mask-External="
                    + outmask
                    + " --DDESolutions-GlobalNorm=None --DDESolutions-DDModeGrid=AP --DDESolutions-DDModeDeGrid=AP --DDESolutions-DDSols=["
                    + args["h5sols"]
                    + ":"
                    + args["h5solstring"]
                    ...
                    + " --Selection-UVRangeKm="
                    + uvsel
                    + " --GAClean-MinSizeInit=10 --Cache-Reset 1 --Beam-Smooth=1 --Cache-DirWisdomFFTW=. --Predict-ColName='PREDICT_SUB'"
                    + StrBeamAt
                ...
                run(
                    ...
                    + clustercat
                    + " --Beam-CenterNorm="
                    + gethistorykey(outmask, "Beam-CenterNorm")
                    + " --Deconv-Mode HMP --Beam-Model="
                    + gethistorykey(outmask, "Beam-Model")
                    + " --Weight-Robust "
                    + str(robust)
                    + " --Image-NPix="
                    + str(imagenpix)
                    ...
                    + str(imagecell)
                    + " --Facets-NFacets="
                    + gethistorykey(outmask, "Facets-NFacets")
                    + " --Freq-NDegridBand="
                    + gethistorykey(outmask, "Freq-NDegridBand")
                    + " --Beam-NBand="
                    + gethistorykey(outmask, "Beam-NBand")
                    + " --Facets-DiamMax 1.5 --Facets-DiamMin 0.1 --Data-Sort 1 --Cache-Dir=. --Log-Memory 1 --Cache-Weight=reset --Output-Mode="
                    + OutputMode
                    + " --Output-RestoringBeam="
                    + gethistorykey(outmask, "Output-RestoringBeam")
                    + " --Freq-NBand="
                    + gethistorykey(outmask, "Freq-NBand")
                    + " --RIME-DecorrMode=FT --Mask-Auto=1 --Mask-SigTh=5.00 "
                    + " --DDESolutions-GlobalNorm=None --DDESolutions-DDModeGrid=AP --DDESolutions-DDModeDeGrid=AP --DDESolutions-DDSols=["
                    + args["h5sols"]
                    + ":"
                    + args["h5solstring"]
            ...
            if not args["useHMP"]:
                ...
                run(
                    "DDF.py --Output-Name=image_full_ampphase_di_m.NS_SUB --Data-ChunkHours="
                    + str(args["chunkhours"])
                    + " --Data-MS="
                    + args["mslist"]
                    + " --Deconv-PeakFactor 0.001000 --Data-ColName "
                    ...
                    + " --Selection-UVRangeKm="
                    + uvsel
                    + " --GAClean-MinSizeInit=10 --Cache-Reset 1 --Beam-Smooth=1 --Predict-ColName='PREDICT_SUB' --Cache-DirWisdomFFTW=. --DDESolutions-SolsDir=SOLSDIR"
                    + StrBeamAt
                    + " --Misc-ConserveMemory 1"
                )
            else:
                ...
                run(
                    ...
                    + " --Selection-UVRangeKm="
                    + uvsel
                    + " --GAClean-MinSizeInit=10 --Cache-Reset 1 --Cache-DirWisdomFFTW=. --Beam-Smooth=1 --Predict-ColName='PREDICT_SUB'"
                    + StrBeamAt
                    + " --Misc-ConserveMemory 1"
                )
        ...
        run(
            "DDF.py --Output-Name=image_full_ampphase_di_m.NS_SUB --Data-MS="
            + args["mslist"]
            + " --Deconv-PeakFactor 0.001000 --Data-ColName "
            + args["column"]
            + " --Parallel-NCPU="
            ...
            + " --DDESolutions-GlobalNorm=None --DDESolutions-DDModeGrid=AP --DDESolutions-DDModeDeGrid=AP --DDESolutions-DDSols="
            + ddsolstr
            + " --DDESolutions-SolsDir=SOLSDIR --Predict-InitDicoModel="
            + outdico
            + " --Selection-UVRangeKm="
            + uvsel
            + " --GAClean-MinSizeInit=10 --Cache-Reset 1 --Beam-Smooth=1 --Cache-DirWisdomFFTW=. --Predict-ColName='PREDICT_SUB'"
    ...
    for ms in msfiles:
        ...
        if ("PREDICT_SUB" in colnames) and (args["column"] in colnames):
            ...
            print(
                "Warning, ",
                ms,
                " does not contain PREDICT_SUB and/or "
                + args["column"]
                + ", skipping.....",
            )
...
if composite:
    print(
        "Since you are using a composite DS9 region file I am not going to phaseshift"
    )
    dophaseshift = False
    # sys.exit()
...
if args["noconcat"]:
    ...
    for ms in msfiles:
        ...
        if dophaseshift:
            ...
            cmd += (
                "phaseshift.type=phaseshift phaseshift.phasecenter=" + phasecenter + " "
            )
        else:
            cmd += "steps=[average] "
        cmd += (
        ...
        cmd += (
            "msout="
            + msout
            + " msout.storagemanager.databitrate=4 msout.storagemanager.weightbitrate=8 "
        )
...
if dokmscal:
    ...
    run(
        "DDF.py --Output-Name=image_full_ampphase_di_m.NS_TAR --Data-MS="
        + args["mslist"]
        + " --Deconv-PeakFactor 0.001000 --Data-ColName "
        + args["column"]
        + " --Parallel-NCPU="
        ...
        + " --Beam-CenterNorm=1 --Deconv-Mode SSD --Beam-Model=LOFAR "
        + beamoption
        + " --Weight-Robust -0.500000 --Image-NPix=20000 --CF-wmax 50000 --CF-Nw 100 --Output-Also onNeds --Image-Cell 1.500000 --Facets-NFacets=11 --SSDClean-NEnlargeData 0 --Freq-NDegridBand 1 --Beam-NBand 1 --Facets-DiamMax 1.5 --Facets-DiamMin 0.1 --Deconv-RMSFactor=3.000000 --SSDClean-ConvFFTSwitch 10000 --Data-Sort 1 --Cache-Dir=. --Log-Memory 1 --Cache-Weight=reset --Output-Mode=Predict --Output-RestoringBeam 6.000000 --Freq-NBand=2 --RIME-DecorrMode=FT --SSDClean-SSDSolvePars [S,Alpha] --SSDClean-BICFactor 0 --Mask-Auto=1 --Mask-SigTh=5.00 --Mask-External="
        + outmask
        + " --DDESolutions-GlobalNorm=None --DDESolutions-DDModeGrid=AP --DDESolutions-DDModeDeGrid=AP --DDESolutions-DDSols="
        + solsfile
        + " --Predict-InitDicoModel="
        ...
        + " --Selection-UVRangeKm="
        + uvsel
        + " --GAClean-MinSizeInit=10 --Cache-Reset 1 --Beam-Smooth=1 --Cache-DirWisdomFFTW=. --Predict-ColName='PREDICT_TAR'"
        + " --Misc-ConserveMemory 1"
    )
        ...
        if not args["onlyuseweightspectrum"]:
            ...
            if aoflagger:
                ...
                if takeoutbeam:
                    ...
                    cmd += "applybeam.type=applybeam applybeam.usechannelfreq=True "
                    cmd += (
                        "applybeam.beammode=array_factor applybeam.updateweights=True "
                    )
                else:
                    cmd += "steps=[phaseshift,aoflagger,average] "
            else:
            ...
            if dophaseshift:
                cmd += (
                    "phaseshift.type=phaseshift phaseshift.phasecenter="
                    + phasecenter
                    + " "
                )
            cmd += "average.type=averager "
            ...
            print(" ")
            print(
                "Ouput column WEIGHT_SPECTRUM used for imaging (contains IMAGING_WEIGHT from DR2)"
            )
            print(
                "Ouput column WEIGHT_SPECTRUM_SOLVE used for calibration (contains WEIGHT_SPECTRUM from DR2)"
            )
