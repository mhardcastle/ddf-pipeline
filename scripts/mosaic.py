...
def make_mosaic(args):
    ...
    if args.do_vlow:
        ...
        intname = (
            "image_full_ampphase_di_m.NS_Band%i_shift.int.facetRestored.fits" % band
        )
        appname = (
            "image_full_ampphase_di_m.NS_Band%i_shift.app.facetRestored.fits" % band
        )
    ...
    if args.shift:
        ...
        for d in args.directories:
            t = Table.read(
                d + "/image_full_ampphase1m.cat.fits_FIRST_match_filtered.fits"
            )
    ...
    if bth:
        ...
        for i in range(len(app)):
            ...
            if args.load and os.path.isfile(outname):
                ...
                if args.save:
                    hdus[i].writeto(outname, overwrite=True)
    ...
    if not (args.no_write):
        ...
        hdu.writeto(
            rootname + mosname.replace(".fits", "-weights.fits"), overwrite=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="rootname",
        default="",
        help="Root name for output files, default uses no prefix",
    )
    parser.add_argument(
        "--beamcut", dest="beamcut", default=0.3, help="Beam level to cut at"
    )
    parser.add_argument(
        "--band",
        dest="band",
        default=None,
        help="Band number to mosaic, leave unset for full-bw image",
    )
    parser.add_argument(
        "--exact",
        dest="exact",
        action="store_true",
        help="Do exact reprojection (slow)",
    )
    parser.add_argument(
        "--save", dest="save", action="store_true", help="Save intermediate images"
    )
    parser.add_argument(
        "--load",
        dest="load",
        action="store_true",
        help="Load existing intermediate images",
    )
    parser.add_argument(
        "--noise",
        dest="noise",
        type=float,
        nargs="+",
        help="UNSCALED Central noise level for weighting: must match numbers of maps",
    )
    parser.add_argument(
        "--scale",
        dest="scale",
        type=float,
        nargs="+",
        help="Scale factors by which maps should be multiplied: must match numbers of maps",
    )
    parser.add_argument(
        "--use_shifted",
        dest="use_shifted",
        action="store_true",
        help="Use the shifted images from the pipeline",
    )
    parser.add_argument(
        "--shift",
        dest="shift",
        action="store_true",
        help="Shift images before mosaicing",
    )
    parser.add_argument(
        "--no_write",
        dest="no_write",
        action="store_true",
        help="Do not write final mosaic",
    )
    parser.add_argument(
        "--find_noise",
        dest="find_noise",
        action="store_true",
        help="Find noise from image",
    )
    parser.add_argument(
        "--do_lowres",
        dest="do_lowres",
        action="store_true",
        help="Mosaic low-res images instead of high-res",
    )
    parser.add_argument(
        "--do_vlow",
        dest="do_vlow",
        action="store_true",
        help="Mosaic vlow images instead of high-res",
    )
    parser.add_argument(
        "--do_wsclean",
        dest="do_wsclean",
        action="store_true",
        help="Mosaic subtracted vlow images instead of high-res",
    )
    parser.add_argument(
        "--do_stokesV",
        dest="do_stokesV",
        action="store_true",
        help="Mosaic stokes V images instead of high-res",
    )
    ...
    parser.add_argument(
        ...
        dest="load_layout",
        action="store_true",
        help="Load a previously defined mosaic layout rather than determining from the images.",
    )
